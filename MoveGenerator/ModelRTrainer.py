...
decisions_ = ["chow_-1", "chow_0", "chow_1", "pong", "no_action"]
n_decisions = 34 + len(decisions_)

class ModelRTrainer(MoveGenerator):
	def __init__(self, player_name, model, **kwargs):
		super(ModelRTrainer, self).__init__(player_name, display_tgboard = False)
	...
	def decide_chow(self, player, new_tile, choices, neighbors, game):
		...
		action = 34 + decisions_.index("no_action") if not is_chow else 34 + decisions_.index("chow_%d"%choice)
		h_action = 34 + decisions_.index("no_action") if not h_is_chow else 34 + decisions_.index("chow_%d"%h_choice)
		if self.__is_train:
	...
	def decide_kong(self, player, new_tile, kong_tile, location, src, neighbors, game):
		...
		action = 34 + decisions_.index("no_action") if not is_kong else 34 + decisions_.index("pong")
		h_action = 34 + decisions_.index("no_action") if not h_is_kong else 34 + decisions_.index("pong")
		if self.__is_train:
			self.__model.update_history(state, action, action_filter, heuristics_action = h_action)
	...
	def decide_pong(self, player, new_tile, neighbors, game):
		...
		action = 34 + decisions_.index("no_action") if not is_pong else 34 + decisions_.index("pong")
		h_action = 34 + decisions_.index("no_action") if not h_is_pong else 34 + decisions_.index("pong")
		if self.__is_train:
			self.__model.update_history(state, action, action_filter, heuristics_action = h_action)
