...
class TGPlayer(Player):
	...
	def new_turn(self, new_tile, neighbors, game, response = None):
		...
		if substate is None or substate == "new_turn_drop_tile":
			substate = None
			dispose_tile = self.__move_generator.decide_drop_tile(self, new_tile, neighbors, game, response = response)
			
			if isinstance(dispose_tile, TGResponsePromise):
				dispose_tile.push_state_stack_push("new_turn_drop_tile")
				return dispose_tile
	...
	def check_chow(self, new_tile, neighbors, game, response = None):
		...
		if criteria:
			is_able = True
			is_wants_to, which = self.__move_generator.decide_chow(self, new_tile, choices, neighbors, game, response = response)
	...
	def check_new_tile_kong(self, new_tile = None, search_hand = "", src = "", neighbors = None, game = None, response = None):
		...
		if is_able:
			is_wants_to = self.__move_generator.decide_kong(self, new_tile, new_tile, location, src, neighbors, game, response = response)
			if isinstance(is_wants_to, TGResponsePromise):
	...
	def check_pong(self, new_tile, neighbors, game, response = None):
		if self.get_tile_hand_count(new_tile) < 2:
			...
			is_wants_to = self.__move_generator.decide_pong(self, new_tile, neighbors, game, response = response)
	...
	def check_win(self, new_tile, tile_src, neighbors, game, response = None):
		...
		if grouped_hand is not None:
			is_wants_to = self.__move_generator.decide_win(self, grouped_hand, new_tile, tile_src, score, neighbors, game, response = response)
